...
using System.Net;
using Report_Deployment_Wizard.ReportService2010;
...
namespace ReportDeployment.GUI
{
    static class Program 
    {
	     ...
	     * Return:	bool	
	     */
        public static ErrorInfo CreateFolder()
        {
            ...
            catch (Exception e)
            {
                ...
                //MessageBox.Show("LYNQ folder already exists", "Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        public static void deployReports (string txtReportingServerURL)
        {
            ...
            cmdProcess.Start();
            cmdProcess.StandardInput.WriteLine("cd "+@"C:\Program Files\Microsoft SQL Server Reporting Services\Shared Tools");
            cmdProcess.StandardInput.Flush();
            cmdProcess.StandardInput.WriteLine("rs.exe -i \"" + @"C:\Tools\RDL\rss\deploy.rss" + "\" -s " + txtReportingServerURL);
	     ...
	     * Return:		bool
	     */
        public static bool checkForSSRS() 
         {
	     ...
	     * Return:		bool
	     */
        public static bool checkDatabaseConnection(string txtServer, string txtDatabase, string txtUserName, string txtPassword, int mode)
        {
            ...
            try
            {
                ...
                if (helper.isConnection)
                    if(mode == 1)
                        MessageBox.Show("ERP Database Connection successful", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    else
                        MessageBox.Show("LYNQ Database Connection successful", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return true;
            }
            catch (Exception ex)
            {
	     ...
	     * Return:		bool
	     */
        public static bool checkURL (string txtReportingServerURL)
        {
            try
            {
                ...
                if (!temp)
                    MessageBox.Show("Failed connecting to reporting server, invalid url");
                response.Close();
                return temp;
            }
            catch(Exception ex)
